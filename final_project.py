...
def filter_data(selected_type):
    ...
    #Count number of each business type
    type_counts = df['app_license_category'].value_counts()

    #Identify types with fewer than 10
    less_than_10 = type_counts[type_counts < 10].index.tolist()

    if selected_type == 'All':
        return df
    elif selected_type in specific_types:
        return df[df['app_license_category'] == selected_type]
    else:  # 'Other' - business types with less than 10 locations
        return df[df['app_license_category'].isin(less_than_10)]

#Function to plot business types
def plot_business_type_distribution(data):
    less_common_types = get_less_common_types(data)

    #Create new column for adjusted business type
    data['adjusted_business_type'] = data['app_license_category']
    data.loc[data['app_license_category'].isin(less_common_types), 'adjusted_business_type'] = 'Other'

    #Count each adjusted business type
    business_counts = data['adjusted_business_type'].value_counts()

    #Create the pie chart
    fig, ax = plt.subplots()
    ax.pie(business_counts, labels=business_counts.index, autopct='%1.1f%%', startangle=90)
    ax.axis('equal')
...
#Function to identify business types with fewer than 10
def get_less_common_types(data, threshold=10):
    type_counts = data['app_license_category'].value_counts()
    return type_counts[type_counts < threshold].index.tolist()
...
def show_data_page():
    ...
    #Identify less common business types
    less_common_types = get_less_common_types(df)

    #Apply filters based on what is chosen by user
    filtered_data = df.copy()
    if selected_type == 'Other':
        filtered_data = filtered_data[filtered_data['app_license_category'].isin(less_common_types)]
